...
const objBTN = document.getElementById("btnSearch");
const objRESULT = document.getElementById("result-container");
...
function searchData(title, location){
  ...
  let result = jobs.filter(worker => worker.location.toLowerCase().includes(location) && worker.title.toLowerCase().includes(title));
...
function checkData(){
  ...
  /*
  if (objJOB.value ==='' && objLOCATION.value ===''){
...
function resetFields(){
  ...
  objJOB.value ='';
  objLOCATION.value ='';
  objTOTALRESULT.innerText ='';

}


objBTN.addEventListener("click", checkData)

objRESET.addEventListener("click", resetFields);